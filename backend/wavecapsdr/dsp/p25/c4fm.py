...
class _Equalizer:
    ...
    def get_equalized_symbol(self, buffer: NDArrayAny, offset: int, mu: float) -> float:
...
class C4FMDemodulator:
    ...
    def demodulate(self, iq: NDArrayComplex) -> tuple[NDArrayInt, NDArrayFloat]:
        ...
        with _c4fm_profiler.measure("fm_demod"):
            phases = self._fm_demod.demodulate(i_rrc.astype(np.float32), q_rrc.astype(np.float32))
        ...
        for sym_idx, soft_symbol_norm in enumerate(soft_symbols_arr):
            ...
            if self._fine_sync:
                ...
                if (
                    sync_score_lag > sync_score_primary
                    and sync_score_lag >= self.SYNC_THRESHOLD_DETECTION
                ):
    ...
    def demodulate_discriminator(self, disc_audio: NDArrayFloat) -> tuple[NDArrayInt, NDArrayFloat]:
        ...
        with _c4fm_profiler.measure("rrc_filter"):
            ...
            disc_rrc, self._rrc_state_disc = signal.lfilter(
                self._rrc_filter, 1.0, disc_audio.astype(np.float32), zi=self._rrc_state_disc
            )
