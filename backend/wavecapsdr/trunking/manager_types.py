...
class TrunkingManagerLike(Protocol):
    ...
    async def add_system(self, config: "TrunkingSystemConfig", talkgroups_csv: str | None = None) -> TrunkingSystemLike: ...
    async def remove_system(self, system_id: str) -> None: ...
    async def update_talkgroups(self, system_id: str, talkgroups: list["TalkgroupConfig"]) -> tuple[int, int]: ...
    async def clear_messages(self, system_id: str) -> int: ...
    async def set_hunt_mode(self, system_id: str, mode: "HuntMode", locked_freq: float | None = None) -> None: ...
